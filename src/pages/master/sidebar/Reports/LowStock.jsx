...
// src/pages/master/sidebar/Reports/LowStock.jsx
...
// Reuse the same helper logic / UX as TopSellingProducts.jsx
...
export default function LowStock() {
  ...
  // date filter UI state (global) - default All
...
const [modalTotal, setModalTotal] = useState(0);
  ...
  const fetchLowStock = async ({
    ...
    try {
      ...
      const res = await axios.get(
        `${API}/api/reports/low-stock/all-shops`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: {
            ...(from && to && {
      ...
      const shopData = res.data?.shops?.[0]; // 🔥 ONLY ONE SHOP

      if (!shopData) return;

      // ✅ UPDATE ONLY THIS SHOP
  ...
  // const fetchLowStock = async ({ search = "", shopname = "" } = {}) => {
  //   try {
  //     const token = getToken();
  //     if (!token) return;
  ...
  //     setShopsWithProducts(formatted);
  ...
  //   if (shops.length) {
  //     fetchLowStock();
  //     const id = setInterval(fetchLowStock, 10000);
  ...
  const handlePageChange = (shop, page) => {
    fetchLowStock({
      shopId: shop._id,
  ...
  const handleSearchChange = (shop, value) => {
  ...
  // Modal open - fetch full list for the shop (backend returns aggregated products)
  ...
  //     if (from && to) cfg.params = { from: from.toISOString(), to: to.toISOString() };

  //     const res = await axios.get(
  //       `${API}/api/shops/${encodeURIComponent(shop.shopname)}/report/products/low-stock/recent`,
  //       cfg
  //     );
  ...
  const fetchModalData = async (page = 1, search = "") => {
    try {
      const token = getToken();
  ...
  const handleLowStockPrint = async () => {
    try {
      const token = getToken();
      if (!token || !modalShop) return;
      ...
      await axios.get(
        `${API}/api/shops/${encodeURIComponent(
          modalShop.shopname
        )}/report/products/low-stock/recent/print`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: buildLowStockParams(),
        }
      );
    } catch (err) {
      console.error("❌ Low Stock Print failed", err);
      alert("Low Stock print failed");
    }
  };
  ...
  const handleLowStockPdf = async () => {
    try {
      ...
      const res = await axios.get(
        `${API}/api/shops/${encodeURIComponent(
          modalShop.shopname
        )}/report/products/low-stock/recent/pdf`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: buildLowStockParams(),
          responseType: "blob",
      ...
      a.href = url;
      a.download = `${modalShop.shopname}_Low_Stock_Report.pdf`;
      ...
      console.error("❌ Low Stock PDF failed", err);
      alert("Low Stock PDF failed");
    }
  };
  ...
  const handleConfirmAction = async () => {
    try {
      ...
      if (confirmType === "print") {
        await handleLowStockPrint();
      } else {
        await handleLowStockPdf();
      }
  ...
  return (
    <div className="pt-10 sm:pt-10">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6   text-[28px]  text-[#00a76f]">Low Stock</h1>
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: "#15803d", margin: 0 }}>{shop.shopname}</h2>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      handleSearchChange(shop, value); // 🔥 pass full shop
                    }}
                    style={{
                      fontSize: 13,
                      padding: "6px 8px",
              ...
              {products.length > 0 ? (
                  ...
                  <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, tableLayout: "fixed" }}>
                    <thead style={{ display: "table", width: "100%", tableLayout: "fixed" }}>
                      <tr style={{ background: "#f3f4f6" }}>
                        {["S.No", "Product Code", "Product Name", "Total Qty", "Min Qty"].map((h, idx) => {
                          const widths = ["10%", "20%", "40%", "15%", "15%"];

                          return (
                            <th
                              key={h}
                              style={{
                                width: widths[idx], // ✅ FIX
                                textAlign: h === "Total Qty" || h === "Min Qty" ? "right" : "left",
                                padding: "10px",
                                fontWeight: 700,
                    ...
                    >
                      {products.map((p, i) => (
                        <tr key={p._id} style={{
                          ...
                          <td style={{ width: "10%" }}>{((page - 1) * 100) + i + 1}</td>
                          <td style={{ width: "20%" }}>{p.code}</td>
                          <td style={{ width: "40%", wordBreak: "break-word", whiteSpace: "normal" }}>{p.name}</td>
                          <td style={{ width: "15%", textAlign: "right", color: p.totalQty <= p.minQty ? "#dc2626" : "#111" }}>{p.totalQty}</td>
                          <td style={{ width: "15%", textAlign: "right" }}>{p.minQty}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div style={{ fontSize: 14, color: "#888" }}>No low-stock products</div>
              )}
            </div>
          );
        })}
      ...
      {modalShop && (
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{modalShop.shopname} — Low Stock</h3>
              ...
              <>
                ...
                >
                  ...
                  <tbody
                    style={{
                      display: "block",
                      maxHeight: "calc(90vh - 250px)", // 🔥 dynamic
                      overflowY: "auto",
                      width: "100%",
                    }}
                  >
                    {modalProducts.length > 0 ? (
                        ...
                        <tr key={p._id} style={{
                          ...
                          <td style={{ width: "15%", padding: "12px 10px", textAlign: "right", color: p.totalQty <= p.minQty ? "#dc2626" : "#111" }}>{p.totalQty}</td>
                          <td style={{ width: "15%", padding: "12px 10px", textAlign: "right" }}>{p.minQty}</td>
                        </tr>
                      ))
                    ) : (
                      <tr
                ...
                >
                  ...
                  <div style={{ color: "#666", fontSize: 13 }}>
                    ...
                    {Math.min(modalPage * 200, modalProducts.length)} of{" "}
