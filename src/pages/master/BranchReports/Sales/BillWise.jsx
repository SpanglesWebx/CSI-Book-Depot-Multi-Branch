...
//src/pages/master/BranchReports/Sales/BillWise.jsx
...
export default function SalesReports() {
  ...
  // 🔥 SAME API PATH LOGIC AS PRODUCTS PAGE
  ...
  const fetchBills = async (p = 1, customLimit = limit) => {
    try {
      setLoading(true);

      const params = { page: p, limit: customLimit };
      ...
      const url = getApiPath("branch-reports/salesbills", user, selectedShop?.shopname);
  ...
  const handlePrint = async () => {
    ...
    const res = await axios.get(
      getApiPath("branch-reports/salesbills"),
      {
    ...
    // 2️⃣ Print (MASTER / TENANT auto-resolved)
    await axios.post(
      getApiPath("master/sales/billwise/print"),
      {
        shopname,
  ...
  const handlePdfDownload = async () => {
    try {
      ...
      const pdfRes = await axios.post(
        getApiPath("master/sales/billwise/pdf"),
        {
          shopname,
          periodLabel: getPeriodLabel(),
          bills: res.data.bills,
  ...
  onChange={(e) => {
    ...
    setLimit(newLimit);
    setPage(1);
    fetchBills(1, newLimit);   // ⭐ send new limit
  }}
