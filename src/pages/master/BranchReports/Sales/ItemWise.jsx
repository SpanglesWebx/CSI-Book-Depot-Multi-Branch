...
const PER_PAGE = 25; // per requirement: 5 cards / rows per page
...
export default function ItemWise() {
  ...
  const fetchSalesMode = async (pageNum = page) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    ...
    if (mode === "sales") fetchSalesMode(page);
    if (mode === "datewise") fetchDateWise(page);

  }, [page]);

  const handleSearchClick = () => {
    setPage(1);
    if (mode === "bill") fetchBillMode();
  ...
  const handlePrint = async () => {
    ...
    // ✅ FETCH DATA (branch-reports ONLY)
    const res = await axios.get(
      buildApiUrl(
        mode === "bill"
          ? "branch-reports/itemwise/bills"
          : mode === "sales"
            ? "branch-reports/itemwise/sales"
            : "branch-reports/datewise",
        user,
        shopname
      ),
      {
        ...
        <div className="grid sm:grid-cols-4 gap-4">
          ...
          <div className="flex flex-col justify-end">
            <div className="flex gap-2 justify-end">
              <button onClick={handleSearchClick} className=" px-4 py-2  flex items-center gap-2 
                    rounded-lg     
    font-semibold
    text-[#007867]
    bg-[#c8fad6]     
                    ...
                    <div className="space-y-8">
                      ...
                      {billList.map((bill, index) => (
                        <div key={bill._id} className="border-b pb-6">
                          ...
                          {/* ⭐ Bill Header — Single Row */}
                          <div className="flex flex-wrap gap-3 text-sm font-semibold text-gray-800 mb-3">

                            <span className="text-green-700">Counter - {bill.counter}</span>
