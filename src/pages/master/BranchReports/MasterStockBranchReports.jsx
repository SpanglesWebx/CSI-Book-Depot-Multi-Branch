// src/pages/master/BranchReports/MasterStockReports.jsx
...
const StockReports = () => {
  ...
  const fetchCategoryProductsAll = async (category, page = 1) => {
    try {
      ...
      // // ✅ UI table data
      // setCategoryProducts(res.data.rows || []);

      // // ✅ FULL data for print / pdf
      // setCategoryProductsAll(res.data.allRows || []);
  ...
  const buildExportParams = () => {
    const params = { all: true }; // 🔥 NO pagination
  ...
  const handlePrint = async () => {
    try {
      ...
      await axios.post(
        buildApiUrl("master/stock/print", user, shopname),
        {
          shopname,
          mode,
          periodLabel,
          rows: allRows, // ✅ FULL DATA (no pagination)
        },
        {
          headers: getHeaders(),
        }
      ...
      console.error("❌ Stock print failed:", err);
      alert("Print failed");
    }
  };
  ...
  const handlePdfDownload = async () => {
    try {
      ...
      const res = await axios.post(
        buildApiUrl("master/stock/pdf", user, shopname),
        {
          shopname,
          mode,
          periodLabel,
      ...
      a.download =
        mode === "stock"
          ? "Product_Stock_Report.pdf"
          : "Category_Stock_Report.pdf";

      a.click();
      window.URL.revokeObjectURL(url);
    } catch (err) {
      console.error("❌ Stock PDF failed:", err);
      alert("PDF failed");
    }
  };
              ...
              <div className="bg-white rounded-lg shadow-md p-4 mb-4">
                ...
                <div className="overflow-x-auto">
                          ...
                          >
                            ...
                            <td className="px-4 py-2 text-center no-print">
                              <button
                                ...
                                //   await fetchCategoryProductsAll(row.category);
                                // }}
                  ...
                  <CategoryProductsModal
                    ...
                    rows={categoryProducts}            // UI
                    allRows={categoryProductsAll}      // Print / PDF
                    shopname={shopname}
                  />
