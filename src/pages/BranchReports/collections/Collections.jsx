...
// src/pages/BranchReports/collections/Collections.jsx
...
const buildApiUrl = (path, user, selectedShopName) => {
  ...
  if (!user) return `${API_BASE}/api/${ep}`;
  if (user.role === "tenant") return `${API_BASE}/api/${ep}`;
  if ((user.role === "manager" || user.role === "megaadmin") && selectedShopName) {
    // follow master-style tenant route used elsewhere in codebase
    return `${API_BASE}/api/tenant/shops/${encodeURIComponent(selectedShopName)}/${ep}`;
  }
...
export default function Collections() {
  ...
  const handlePrint = async () => {
    ...
    if (reportType === "counter") {
      ...
      printUrl = "collections/counter/print";
    } else if (reportType === "user") {
      fetchUrl = "branch-reports/collections/user";
      printUrl = "collections/user/print";
    } else {
      fetchUrl = "branch-reports/collections/statement";
      printUrl = "collections/statement/print";
    }


    const res = await axios.get(
      buildApiUrl(fetchUrl, user, shopname),
      { params: buildExportParams(), headers: getHeaders() }
    );
  ...
  const handlePdfDownload = async () => {
    ...
    if (reportType === "counter") {
      fetchUrl = "branch-reports/collections/counter";
      pdfUrl = "collections/counter/pdf";
      fileName = "Counter_Collections.pdf";
    } else if (reportType === "user") {
      fetchUrl = "branch-reports/collections/user";
      pdfUrl = "collections/user/pdf";
      fileName = "User_Collections.pdf";
    } else {
      fetchUrl = "branch-reports/collections/statement";
      pdfUrl = "collections/statement/pdf";
              ...
              <table className="table-auto w-full  border-collapse">
                ...
                <tbody>
                  {counterRows.length === 0 ? (
                    ...
                    // counterRows.slice((page - 1) * limit, (page - 1) * limit + limit).map((r, i) => (
                    counterRows.map((r, i) => (
              ...
              <table className="table-auto w-full border-collapse">
                ...
                <tbody>
                  {userRows.length === 0 ? (
                    ...
                    userRows.slice((page - 1) * limit, (page - 1) * limit + limit).map((r, i) => (
                ...
                <table className="table-auto w-full border-collapse">
                  ...
                  <tbody>
                    {statementCounterRows.length === 0 ? (
                      ...
                      // statementCounterRows.slice((page - 1) * limit, (page - 1) * limit + limit).map((r, i) => (
                      statementCounterRows.map((r, i) => (
                        <tr key={r.counter || i} className="border-t hover:bg-green-50 transition-colors">
                          <td className="px-4 py-2">{(page - 1) * limit + i + 1}</td>
                          {/* <td className="px-4 py-2">Counter - {r.counter}</td> */}
                ...
                <table className="table-auto w-full  border-collapse">
                  ...
                  <tbody>
                    {statementUserRows.length === 0 ? (
                      ...
                      // statementUserRows.slice((page - 1) * limit, (page - 1) * limit + limit).map((r, i) => (
                      statementUserRows.map((r, i) => (
                        <tr key={r.user || i} className="border-t hover:bg-green-50 transition-colors">
                          <td className="px-4 py-2">{(page - 1) * limit + i + 1}</td>
                          {/* <td className="px-4 py-2">{r.user || r.username || "-"}</td> */}
